...
void setOrder(vector <int> v){
	int i,j,randOrd;
	vector<bool> daXep(numVtx, false);
	// TODO: thuat toan hoan vi
	for(i =numVtx-2; i>0;i--){
		if(hasDeg[i].size() >0){
			for(j=0; j<hasDeg[i].size(); j++){
				do {
					randOrd = rand()% (hasDeg[i].size());
				}
				while(daXep[hasDeg[i][randOrd]] == true);
				v.push_back(hasDeg[i][randOrd]);
				cout<<hasDeg[i][randOrd]<<" ";
				daXep[hasDeg[i][randOrd]]=true;
			}
		}
	}
}
...
void colorGraph(int numColor,vector <int> v){
	int i;
	daToMau.resize(numVtx, false);
	colorOfVtx.resize(numVtx,NULL);
	colorOfVtx[v[0]]=0;
	daToMau[v[0]]=true;
